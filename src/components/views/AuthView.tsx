...
export const AuthView: React.FC<AuthViewProps> = ({ mode, onNavigate, onAuthenticated }) => {
  ...
  return <main className="min-h-[70vh] flex items-center justify-center p-6"><section className="w-full max-w-md rounded-2xl border border-[#292929] bg-[#0d0d0d] p-6 sm:p-8 space-y-6" aria-labelledby="auth-title">
    ...
    <p className="text-center text-xs text-neutral-500">{isRegister ? 'Já possui uma conta?' : 'Ainda não possui uma conta?'} <button type="button" onClick={() => onNavigate(isRegister ? '/auth/login' : '/auth/register')} className="text-[#E00000] underline">{isRegister ? 'Entrar' : 'Criar conta'}</button></p>
  </section></main>;
};
