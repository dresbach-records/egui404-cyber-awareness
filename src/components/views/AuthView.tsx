...
export const AuthView: React.FC<AuthViewProps> = ({ mode, onNavigate, onAuthenticated }) => {
  ...
  if (isRecovery) return <CommunityAuthShell title={mode === 'forgot' ? 'Esqueci minha senha' : 'Redefinir senha'} onNavigate={onNavigate}><section className="space-y-5 rounded-2xl border border-[#292929] bg-[#0d0d0d] p-6 sm:p-8" aria-labelledby="auth-title"><p className="text-sm leading-6 text-neutral-400">A recuperação depende do contrato de autenticação do backend. Nenhum e-mail será enviado nesta etapa.</p><label className="block text-xs text-neutral-400">E-mail<input required type="email" value={recoveryEmail} onChange={(e) => setRecoveryEmail(e.target.value)} className="mt-1.5 w-full rounded-lg border border-[#292929] bg-[#141414] px-3 py-3 text-sm text-white outline-none focus:border-[#E00000]" /></label><div className="rounded-lg border border-[#5c3b18] bg-[#291b0c] p-3 text-xs leading-5 text-[#f2c98b]">Endpoint real pendente. O fluxo não foi simulado.</div><button type="button" onClick={() => onNavigate('/auth/login')} className="w-full rounded-lg border border-[#292929] px-4 py-3 text-sm text-white hover:border-[#E00000]">Voltar para entrar</button></section></CommunityAuthShell>;
