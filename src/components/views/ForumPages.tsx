...
export const ForumPages: React.FC<ForumPagesProps> = ({ path, onNavigate }) => {
  ...
  if (path === '/forum/communities') return <PageShell title="Comunidades" eyebrow="Explorar" onNavigate={onNavigate}><div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_320px]"><BackendState label="Comunidades da API" icon={<Users className="h-5 w-5" />} /><aside className="rounded-lg border border-[#332020] bg-[#160b0b] p-5"><p className="text-[10px] font-mono uppercase tracking-[0.2em] text-[#FF5A5A]">Comunidade defensiva</p><h2 className="mt-3 text-lg font-semibold text-white">Espaço seguro para aprender</h2><p className="mt-2 text-sm leading-6 text-[#A1A1AA]">Discussões sobre segurança digital com responsabilidade, evidências e respeito.</p><button type="button" onClick={() => onNavigate('/forum/rules')} className="mt-4 text-sm text-[#FF5A5A] hover:underline">Ver regras →</button></aside></div></PageShell>;
