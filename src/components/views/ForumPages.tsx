...
const BackendState: React.FC<{ label: string; icon: React.ReactNode; description?: string; action?: string; onAction?: () => void }> = ({ label, icon, description, action, onAction }) => (
  <section className="rounded-lg border border-[#252525] bg-[#0B0B0B] p-8 text-center">
    ...
    <p className="mx-auto mt-2 max-w-md text-sm leading-6 text-[#A1A1AA]">{description || 'Aqui você encontra as atualizações e interações da comunidade.'}</p>
...
export const ForumPages: React.FC<ForumPagesProps> = ({ path, onNavigate }) => {
  ...
  if (sessionState === 'checking') return <div className="min-h-[75vh] bg-[#050505] p-12 text-center text-sm text-[#A1A1AA]"><div className="mx-auto h-8 w-8 animate-spin rounded-full border-2 border-[#333] border-t-[#E00000]" /><p className="mt-4">Carregando sua comunidade</p></div>;
  if (sessionState === 'guest') return <PageShell title="Entre para participar" eyebrow="Comunidade E GUI 404" onNavigate={onNavigate}><section className="max-w-xl rounded-lg border border-[#252525] bg-[#0B0B0B] p-8"><p className="text-sm leading-6 text-[#A1A1AA]">Faça login para publicar, comentar, salvar discussões e acompanhar suas notificações.</p><div className="mt-6 flex flex-wrap gap-3"><button type="button" onClick={() => onNavigate('/auth/login')} className="rounded bg-[#E00000] px-4 py-2 text-sm font-semibold text-white">Entrar</button><button type="button" onClick={() => onNavigate('/auth/register')} className="rounded border border-[#333] px-4 py-2 text-sm text-white">Criar conta</button></div></section></PageShell>;
  ...
  if (path === '/forum/create') return <PageShell title="Criar publicação" eyebrow="Comunidade" onNavigate={onNavigate}><div className="rounded-lg border border-[#252525] bg-[#0B0B0B] p-6"><p className="max-w-xl text-sm leading-6 text-[#A1A1AA]">Escolha uma comunidade e publique uma discussão com contexto, fontes e segurança.</p><button type="button" onClick={() => onNavigate('/forum')} className="mt-5 rounded bg-[#E00000] px-5 py-3 text-sm font-semibold text-white hover:bg-[#b80000]">Abrir editor</button></div></PageShell>;
  if (path === '/forum/popular' || path === '/forum/trending') return <PageShell title={path.endsWith('trending') ? 'Em alta' : 'Publicações populares'} eyebrow="Descoberta" onNavigate={onNavigate}><BackendState label="Discussões populares" icon={<TrendingUp className="h-5 w-5" />} /></PageShell>;
  if (path === '/forum/recent') return <PageShell title="Publicações recentes" eyebrow="Atualizações" onNavigate={onNavigate}><BackendState label="Publicações recentes" icon={<FileText className="h-5 w-5" />} /></PageShell>;
  if (path === '/forum/explore') return <PageShell title="Explorar" eyebrow="Descoberta" onNavigate={onNavigate}><BackendState label="Descobertas para você" icon={<Compass className="h-5 w-5" />} /></PageShell>;
  if (path === '/forum/history') return <PageShell title="Histórico" eyebrow="Sua atividade" onNavigate={onNavigate}><BackendState label="Seu histórico" icon={<FileText className="h-5 w-5" />} /></PageShell>;
  if (path === '/forum/communities') return <PageShell title="Comunidades" eyebrow="Explorar" onNavigate={onNavigate}><div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_320px]"><BackendState label="Comunidades recomendadas" icon={<Users className="h-5 w-5" />} /><aside className="rounded-lg border border-[#332020] bg-[#160b0b] p-5"><p className="text-[10px] font-mono uppercase tracking-[0.2em] text-[#FF5A5A]">Comunidade defensiva</p><h2 className="mt-3 text-lg font-semibold text-white">Espaço seguro para aprender</h2><p className="mt-2 text-sm leading-6 text-[#A1A1AA]">Discussões sobre segurança digital com responsabilidade, evidências e respeito.</p><button type="button" onClick={() => onNavigate('/forum/rules')} className="mt-4 text-sm text-[#FF5A5A] hover:underline">Ver regras →</button></aside></div></PageShell>;
  if (path === '/forum/categories/create') return <PageShell title="Criar categoria" eyebrow="Categorias" onNavigate={onNavigate}><form className="max-w-xl space-y-4 rounded-lg border border-[#252525] bg-[#0B0B0B] p-5" onSubmit={(event) => event.preventDefault()}><label className="block text-sm text-white">Nome da categoria<input required className="mt-2 w-full rounded border border-[#333] bg-[#050505] p-3 text-sm outline-none focus:border-[#E00000]" placeholder="Ex.: Segurança mobile" /></label><label className="block text-sm text-white">Descrição<textarea className="mt-2 min-h-28 w-full rounded border border-[#333] bg-[#050505] p-3 text-sm outline-none focus:border-[#E00000]" placeholder="Explique o propósito da categoria" /></label><button type="submit" className="rounded bg-[#E00000] px-4 py-2 text-sm font-semibold text-white">Criar categoria</button></form></PageShell>;
  if (path.startsWith('/forum/category/')) return <PageShell title={path.endsWith('all') ? 'Todas as categorias' : path.endsWith('security') ? 'Segurança digital' : 'Golpes e fraudes'} eyebrow="Categorias" onNavigate={onNavigate}><BackendState label="Publicações da categoria" icon={<Compass className="h-5 w-5" />} /></PageShell>;
  if (path === '/forum/search') return <PageShell title="Pesquisa global" eyebrow="Buscar" onNavigate={onNavigate}><div className="mb-5 flex max-w-2xl items-center gap-3 rounded border border-[#333] bg-[#0B0B0B] px-4 py-3"><Search className="h-4 w-4 text-[#E00000]" /><input className="w-full bg-transparent text-sm outline-none" placeholder="Pesquisar posts, comunidades, usuários e tags" /></div><BackendState label="Resultados encontrados" icon={<Search className="h-5 w-5" />} /></PageShell>;
  ...
  if (path === '/forum/notifications') return <PageShell title="Notificações" eyebrow="Atividade" onNavigate={onNavigate}><BackendState label="Suas notificações" icon={<Bell className="h-5 w-5" />} /></PageShell>;
  ...
  if (path.startsWith('/forum/u/')) return <PageShell title={`Perfil @${path.split('/').pop()}`} eyebrow="Membro" onNavigate={onNavigate}><BackendState label="Perfil público" icon={<Shield className="h-5 w-5" />} /></PageShell>;
  if (path === '/forum/me' || path === '/forum/me/posts' || path === '/forum/me/comments') return <PageShell title={path.endsWith('posts') ? 'Minhas publicações' : path.endsWith('comments') ? 'Meus comentários' : 'Meu perfil'} eyebrow="Conta" onNavigate={onNavigate}><BackendState label="Atividade do seu perfil" icon={<Users className="h-5 w-5" />} /></PageShell>;
  ...
  if (path === '/forum/moderation') return <PageShell title="Moderação" eyebrow="Área restrita" onNavigate={onNavigate}><BackendState label="Permissões de moderaç��o" icon={<Shield className="h-5 w-5" />} /></PageShell>;
  return null;
};
